...
function renderGrid() {
  ...
  grid.innerHTML = filtered.slice(0, displayed).map((n) => {
    const isFav = favourites.includes(n.nama);
    const idx = namaData.indexOf(n);
    return `<div class="nama-card ${isFav ? 'favourited' : ''}" data-idx="${idx}">
      <div class="nc-top">
        <div class="nc-gender ${n.jantina === 'lelaki' ? 'boy' : 'girl'}">${n.jantina === 'lelaki' ? '👦' : '👧'}</div>
        <button class="nc-fav" data-idx="${idx}">${isFav ? '❤️' : '🤍'}</button>
      </div>
